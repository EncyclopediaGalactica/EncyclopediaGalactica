...
pub async fn exercises_generate_books_scenario(
    input: ExercisesGenerateBooksScenarioInput,
) -> anyhow::Result<()> {
    debug!(
...
fn validate_scenario_input(input: ExercisesGenerateBooksScenarioInput) -> anyhow::Result<()> {
    if input.book_references.is_empty() {
        return Err(anyhow::anyhow!(
            "At least one book_reference must be provided."
        ));
    }
...
#[derive(Debug, Clone)]
pub struct ExercisesGenerateBooksScenarioInput {
    pub book_catalog_path: String,
    pub generated_tests_path: String,
    pub db_connection_string: String,
    pub book_references: Vec<String>,
