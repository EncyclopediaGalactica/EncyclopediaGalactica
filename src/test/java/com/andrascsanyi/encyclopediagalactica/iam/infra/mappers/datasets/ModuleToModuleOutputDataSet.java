...
public class ModuleToModuleOutputDataSet implements ArgumentsProvider {
    ...
    public Stream<? extends Arguments> provideArguments(ExtensionContext context) throws Exception {
        return Stream.of(
            ...
            Arguments.of(
                ...
                ModuleOutput.builder().id(0L).name("").description("").build()
            ),
            Arguments.of(
                Module.builder().id(100L).name("  ").description("  ").build(),
                ModuleOutput.builder().id(100L).name("  ").description("  ").build()
            )
        );
    }
}
